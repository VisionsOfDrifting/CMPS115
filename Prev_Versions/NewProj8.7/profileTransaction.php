...
$stmt = $conn->prepare("SELECT id, tr_name, tr_type, tr_cost, total, date FROM budget WHERE name = '$temp' ORDER BY date"); 
$stmt->execute();
...
while($row = $stmt->fetch(PDO::FETCH_ASSOC))
{
	$id_num =  $row['id'];
	if($row['tr_cost'] == '-1'){
		$total_place = $row['total'];
		$query3 = "UPDATE budget SET tr_cost = '$total_place' WHERE id = '$id_num'";
		$bool = $mysqli->query($query3);
		$total_place = $last_total - $total_place;
		$query3 = "UPDATE budget SET total = '$total_place' WHERE id = '$id_num'";
		
		$last_total_new = $total_place;
	} else if($row['tr_cost'] == '-2'){
		$total_place = '0';
		$query3 = "UPDATE budget SET tr_cost = '$total_place' WHERE id = '$id_num'";
		$bool = $mysqli->query($query3);
		$total_place = $row['total'] + $last_total_new;
		$query3 = "UPDATE budget SET total = '$total_place' WHERE id = '$id_num'";
		
		$last_total_new = $total_place;
	} else if($row['tr_cost'] == '0'){
	   $placehold = $row['total'] - $previous_total - $overflow;
	   if($marker == '0'){
		   $marker == '1';
	   } else {
		   $overflow = $overflow + $previous_total - $last_total - $placehold;
	   }  
	   $total_place = $placehold + $last_total_new;
	   
	   $previous_total =  $row['total'];
	   $query3 = "UPDATE budget SET total = '$total_place' WHERE id = '$id_num'";
	   
	   $last_total = $total_place;
	   $last_total_new = $total_place;
   } else {
	   $previous_total =  $row['total'];
...
<div class="w3-main" style="margin-left:250px">
  ...
  <?php include 'css/css.html'; ?>
	  ...
	  <div class="form">
          <h1><?php echo  " The following Transaction succeded" ?></h1> <br>
          <h2><?php echo  " $name-$tr_name-$tr_type-$tr_cost-$total-$tr_date" ?></h2>
