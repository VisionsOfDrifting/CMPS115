...
$start = $mysqli->escape_string($_POST['start']);
$end = $mysqli->escape_string($_POST['end']);
$temp = "";
...
<div class="w3-top">
  <div class="w3-bar w3-theme w3-top w3-left-align w3-large">
    ...
    <a href="#" class="w3-bar-item w3-button w3-hide-small w3-hover-white">Description of the Project</a>
    <a href="#" class="w3-bar-item w3-button w3-hide-small w3-hover-white">Team Project</a>
...
$stmt = $conn->prepare("SELECT tr_name, tr_type, tr_cost, total, date FROM budget WHERE name = '$temp'"); 
$stmt->execute();


if(empty($stmt)){
	echo "<table style='border: solid 1px black;'>";
	echo "<tr><th>Name</th><th>Type</th><th>Cost</th> <th>Balance</th><th>Date<th></tr>";
} else {
...
try {
    $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $conn->prepare("SELECT tr_name, tr_type, tr_cost, date FROM budget WHERE name = '$temp' AND date BETWEEN '$start' and '$end'" ); 
    $stmt->execute();
